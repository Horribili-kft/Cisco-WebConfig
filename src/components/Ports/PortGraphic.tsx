...
const PortGraphic: React.FC<PortGraphicProps> = ({ name, type, up, selected, onClick, tip }) => {
  return (
    <div className={tip ? 'tooltip' : ''} data-tip={tip ? tip : ''}>
      <div
        className={`card bg-base-200 m-1 w-[4.5rem] shadow-2xl hover:cursor-pointer hover:brightness-110 active:scale-95 transition-all duration-75 active:brightness-125 ${selected ? 'scale-105 brightness-125' : 'brightness-75'}`}
        onClick={onClick}
      >
        <figure>
          <svg
